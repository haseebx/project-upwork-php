...

{
...

	{
...

		{
			...
			$CHK = qry_run("Select * from ms_prods where mainid = ".ch_lvl('mid')." AND secid = ".ch_lvl('sec')." AND catid = ".ch_lvl('cat')."");
...

			{
				...
				die;
					...
					$IMG = qry_run("Select * from ms_cat where mainid = ".ch_lvl('mid')." AND secid = ".ch_lvl('sec')." AND catid = ".ch_lvl('cat')."");
					...
					$DEL = qry_run("Delete from ms_cat where mainid = ".ch_lvl('mid')." AND secid = ".ch_lvl('sec')." AND catid = ".ch_lvl('cat')."");

					//--- RE-RABKING -----

					$rank = 1;

					$RNK = qry_run("select * from ms_cat where mainid = ".ch_lvl('mid')." AND secid = ".ch_lvl('sec')." order by ranking");
...

					{
...

						{
							...
							$UPD = qry_run("Update ms_cat Set ranking = ".$rank." where catid = ".$rk['catid']."");
...

				{
...

					{
...

						{
							...
							$img3 = cr_image($img4,3000,'W',$ext,$pic3,$Path);

							$img2 = cr_image($img3,1000,'W',$ext,$pic2,$Path);

							$img1 = cr_image($img3,300,'W',$ext,$pic1,$Path);
